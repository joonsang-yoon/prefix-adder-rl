...
object PrefixSearchMain {
  ...
  def run(
    ...
    seed: Int = 1,
    @arg(name = "learning-rate", doc = "Adam learning rate for policy-gradient updates")
    learningRate: Double = 0.02,
    @arg(name = "temperature", doc = "Softmax temperature used when sampling split actions")
    temperature: Double = 1.0,
    @arg(name = "hidden-size", doc = "Hidden width of the tree-contextual pointer actor-critic")
    hiddenSize: Int = 32,
    @arg(name = "gradient-clip", doc = "Global gradient-norm clip applied once per episode")
    gradientClip: Double = 5.0,
    ...
    @arg(name = "dag-summary-mode", doc = "DAG summary mode: usage-weighted or attention-weighted")
    dagSummaryMode: String = NeuralArchitectureConfig.DefaultDagSummaryMode,
    @arg(name = "action-context-mode", doc = "Candidate interaction mode: self-attention or mean-residual")
    ...
    outputRoot: String = "generated/search/deep",
    ...
    val architecture = NeuralArchitectureConfig(
      dagSummaryMode = dagSummaryMode,
      actionContextMode = actionContextMode
    )
    val policy =
      Option(policyInit).map(_.trim).filter(_.nonEmpty) match {
        case Some(path) => NeuralSplitPolicy.load(os.Path(path, repoRootPath), random)
        case None =>
          new NeuralSplitPolicy(
            random = random,
    ...
    SearchSupport.writeRunConfig(
      ...
      ujson.Obj(
        ...
        "hidden_size" -> policy.hiddenSizeValue,
        "gradient_clip" -> policy.gradientClipValue,
        ...
        "architecture" -> policy.architectureConfig.toJson
      )
    )
    ...
    println(
      s"[search] architecture dag_summary=${policy.architectureConfig.normalizedDagSummaryMode} " +
        s"action_context=${policy.architectureConfig.normalizedActionContextMode}"
    )

    PolicyGradientSearchRunner.run[SampledDecision, PolicyTrainingStats](
      ...
      trainingSummary = stats =>
        f"advantage=${stats.advantage}%.4f value=${stats.averageValue}%.4f value_loss=${stats.valueLoss}%.4f entropy=${stats.averageEntropy}%.4f grad=${stats.clippedGradientNorm}%.4f",
      ...
      summaryFields = Seq(
        ...
        "hidden_size" -> ujson.Num(policy.hiddenSizeValue),
        "gradient_clip" -> ujson.Num(policy.gradientClipValue),
        ...
        "policy_architecture" -> ujson.Str("frontier-conditioned-dag-pointer-actor-critic"),
        "architecture" -> policy.architectureConfig.toJson,
